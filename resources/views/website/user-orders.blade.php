...
@section('content')
                ...
                <div class="bg-white shadow-md rounded-2xl p-4 sm:p-6 border border-grey">
                        ...
                        <div class="flex items-center space-x-3">
                            <span class="px-5 py-2 text-gray-600 text-base border border-gray-300 rounded-full">
                                Estimated arrival: <span
                                    class="font-semibold">{{ $order->shipping->first()->expected_delivery_date ?? 'Order not picked' }}</span>
                            </span>
                            @php
                            ...
                            <span
                                class="px-5 py-2 text-base border rounded-full font-semibold 
                    ...
                    <div class="flex items-center justify-center space-x-6 my-6">
                        ...
                        <div class="flex items-center space-x-2 px-4 py-2 bg-gray-100 rounded-full shadow-sm">
                            ...
                            <span
                                class="text-gray-700 text-sm font-medium">{{ $order->address->first()->city }},{{ $order->address->first()->state }}</span>
                        </div>
                    </div>
                            ...
                            <div class="flex items-center bg-gray-100 p-3 rounded-2xl">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-500 font-semibold"
                                        style="font-family: Arial, Helvetica, sans-serif">{{ $item->product->price }}</p>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="flex justify-between">
                        <p class="text-gray-700 font-semibold" style="font-family: Arial, Helvetica, sans-serif">Total:
                            ₹{{ $order->payments->first()->amount ?? null }} ({{ $order->items->count() }} items)</p>
                        <a href="{{ url('order-details/' . $order->order_no) }}"><button
                                class="px-4 py-2 bg-black text-white rounded-2xl">Details</button></a>
                    </div>
                </div>
            @endforeach
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12 mx-12">
        <h1 class="text-center col-span-2 m-5"><strong>No Order Right Now</strong></h1>
    </div>
